...
class PlayState extends MusicBeatState
{
	...
	private var camPing:FlxCamera;
	
	public var discordBG:FlxSprite;
	public var discordText:FlxText;
	public var discordImage:FlxSprite;
	public var discordName:FlxText;
	public var showDiscord:Bool = false;
	...
	override public function create()
	{
		...
		camPing = new FlxCamera();
		camPing.bgColor.alpha = 0;

		FlxG.cameras.reset(camGame);
		FlxG.cameras.add(camHUD);
		FlxG.cameras.add(camPing);
		...
		discordBG = new FlxSprite().makeGraphic(FlxG.width, FlxG.height, 0xFF2C2F33);
		discordText = new FlxText(0, 0, FlxG.width - 40, "", 40);
		discordText.setFormat(Paths.font('text.otf'), 40, FlxColor.WHITE, LEFT);
		discordName = new FlxText(0, 0, FlxG.width - 40, "", 40);
		discordName.setFormat(Paths.font('name.otf'), 40, FlxColor.WHITE, LEFT);
		discordImage = new FlxSprite(0, 0);
		
		discordBG.cameras = [camPing];
		discordText.cameras = [camPing];
		discordName.cameras = [camPing];
		discordImage.cameras = [camPing];
	...
	override public function update(elapsed:Float)
	{
		if (!showDiscord)
		{
			var subtracter:Float = 1 / 60;
			if (discordBG.alpha > 0)
			{
				if (discordBG.alpha - subtracter < 0)
				{
					discordBG.alpha = 0;
				}
				else
				{
					discordBG.alpha -= subtracter;
				}
			}	
		}
		
		discordImage.alpha = discordBG.alpha;
		discordName.alpha = discordBG.alpha;
		discordText.alpha = discordBG.alpha;
	...
	*/
	...
	override function ping()
	{
		...
		discordImage.setGraphicSize(128, 128);
		if (Logger.bulkAnd(curPing, ["member", "author", "content"]) && Logger.bulkAnd(curPing.author, ["id", "avatar", "username"]) && Logger.bulkAnd(curPing.member, ["nick"]))
		{
			var displayShit:String = (curPing.nick != null) ? curPing.nick : curPing.author.username;
			if (!CachedPFP.cachedPFP.exists(curPing.author.id))
			{
				#if sys
				sys.thread.Thread.create(() -> {
				#end
					var http:Http = new Http("https://cdn.discordapp.com/avatars/" + curPing.author.id + "/" + curPing.author.avatar + ".png?size=128");
					http.onBytes = function(daBytes)
					{
						Image.loadFromBytes(daBytes).onComplete(function(img)
						{
							CachedPFP.cachedPFP.set(curPing.author.id, img);
							discordImage.pixels = BitmapData.fromImage(CachedPFP.cachedPFP.get(curPing.author.id));
							discordImage.setGraphicSize(128, 128);
							discordImage.visible = true;
							showDiscordCrap(displayShit, curPing.content);
						}).onError(function(err)
						{
							trace('Bytes -> Image Conversion Failed With Error: ' + err);
						});
					}
					http.onError = function(err)
					{
						onError(displayShit, curPing.content);
					}
					http.request();
				#if sys
				});
			...
			{
				if (Reflect.hasField(discordImage, "pixels") && Reflect.hasField(discordImage.pixels, "image"))
				{
					if (discordImage.pixels.image != CachedPFP.cachedPFP.get(curPing.author.id))
					{
						discordImage.pixels = BitmapData.fromImage(CachedPFP.cachedPFP.get(curPing.author.id));
						trace(CachedPFP.cachedPFP.get(curPing.author.id));
					}
				}
				showDiscordCrap(displayShit, curPing.content);
			}
	...
	public function onError(name:String, message:String):Void
	{
		discordImage.visible = false;
		showDiscordCrap(name, message);
	}
	
	public function showDiscordCrap(name:String, message:String):Void
	{
		discordName.text = name;
		discordText.text = message;
		var halfY:Float = FlxG.height / 2;
		var otherHalfY:Float = 128 + discordText.height;
		halfY -= otherHalfY / 2;
		discordImage.y = halfY;
		discordImage.x = 0;
		discordName.y = halfY;
		discordName.x = 128;
		discordText.y = halfY + 128 / 2;
		discordText.x = 128;
		var whatHigher:Float = (discordName.width > discordText.width) ? discordName.width : discordText.width;
		var daWidth:Int = Math.floor(128 + whatHigher);
		var daHeight:Int = Math.floor(otherHalfY);
		discordBG.setGraphicSize(daWidth, daHeight);
		discordImage.setGraphicSize(128, 128);
		discordBG.y = (FlxG.height / 2 - halfY - daHeight / 2) * -1;
		discordBG.x = (FlxG.width / 2 - daWidth / 2) * -1;
		showDiscord = true;
		discordBG.alpha = 1;
		discordBG.color = 0xFF36393E;
		if (fadeTimer != null)
		{
			fadeTimer.cancel();
			fadeTimer.destroy();
		...
		fadeTimer = new FlxTimer().start(2, function(tmr:FlxTimer)
		{
			showDiscord = false;
		}, 1);
	}
