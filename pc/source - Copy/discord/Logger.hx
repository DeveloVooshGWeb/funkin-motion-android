	...
	public static function onData(daString:String):Void
	{
		if (isJSON(daString))
		{
			...
			{	
				...
				{
					case 'MESSAGE_CREATE':
						if (bulkAnd(daJson.d, ["channel_id", "guild_id"]))
						{
							if ((daJson.d.channel_id == BotData.channel_id || BotData.channel_id == "") && (daJson.d.guild_id == BotData.guild_id || BotData.guild_id == ""))
							{
		...
		{
			socketSend(Json.stringify({ op: 20, token: BotData.token, guild_id: BotData.guild_id, channel_id: BotData.channel_id }));
		}
	}
